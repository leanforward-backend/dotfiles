...
Panel {
  ...
  // -1 = not previewing; track the live value locally while a drag/apply is
  // in flight so the slider doesn't bounce during the hyprctl round-trip
  // (mirrors textSizePreviewIndex above).
  property int nightlightStrengthPreview: -1

  function nightlightTempToPercent(temp) {
    if (temp === null || temp === undefined) return 0
    var pct = Math.round((root.nightlightMaxTemp - temp) / (root.nightlightMaxTemp - root.nightlightMinTemp) * 100)
  ...
  function previewNightlightStrength(pct) {
    root.nightlightStrengthPreview = Math.round(pct)
    nightlightStrengthDebounce.restart()
  }
  ...
  function adjustNightlightStrength(delta) {
    if (focusSection !== "nightlightstrength") return
        ...
        Column {
          ...
          Column {
            ...
            CursorSurface {
              id: nightlightRow
              width: parent.width
              height: nightlightInner.implicitHeight + Style.spacing.xl
              hasCursor: root.cursorActive && root.focusSection === "nightlight" && root.selectedIndex === -1
              onHasCursorChanged: if (hasCursor) root.ensureCursorVisible(nightlightRow)
              current: root.nightlightEnabled
              foreground: root.bar.foreground
            ...
            Item {
              width: parent.width
              implicitHeight: Math.max(nightlightStrengthLabel.implicitHeight, nightlightStrengthPercent.implicitHeight)

              Text {
                id: nightlightStrengthLabel
                text: "Strength"
                color: Qt.darker(root.bar.foreground, 1.4)
                font.family: root.bar.fontFamily
                font.pixelSize: Style.font.caption
              ...
              Text {
                id: nightlightStrengthPercent
                text: Math.round(nightlightStrengthSlider.dragging ? nightlightStrengthSlider.liveValue : root.currentNightlightStrength()) + "%"
                color: Qt.darker(root.bar.foreground, 1.4)
                font.family: root.bar.fontFamily
                font.pixelSize: Style.font.caption
                font.bold: true
            ...
            CursorSurface {
              id: nightlightStrengthRow
              width: parent.width
              height: nightlightStrengthSlider.implicitHeight + Style.spacing.controlGap
              hasCursor: root.cursorActive && root.focusSection === "nightlightstrength" && root.selectedIndex === -1
              onHasCursorChanged: if (hasCursor) root.ensureCursorVisible(nightlightStrengthRow)
              foreground: root.bar.foreground
              outline: true
              opacity: root.nightlightEnabled ? 1.0 : 0.6

              PanelSlider {
                id: nightlightStrengthSlider
                bar: root.bar
                anchors.fill: parent
                anchors.leftMargin: Style.space(6)
                anchors.rightMargin: Style.space(6)
                ...
                maximum: 100
                step: 1
                value: root.currentNightlightStrength()
                integer: true
                onMoved: function(v) { root.previewNightlightStrength(v) }
                onReleased: function(v) { root.setNightlightStrength(v) }
              }

              HoverHandler {
                onHoveredChanged: if (hovered && !root.reflowingText) {
                  root.cursorActive = true
                  root.focusSection = "nightlightstrength"
                  root.selectedIndex = -1
                }
              }
            }
