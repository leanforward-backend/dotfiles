...
function roundedDecimal(value, digits) {
  if (value === undefined || value === null || value === "") return ""
  var n = parseFloat(String(value))
  if (isNaN(n)) return ""
...
function shortTime(value) {
  ...
  return marker >= 0 ? text.slice(marker + 1, marker + 6) : text
}

function compassDirection(value) {
  var degrees = parseFloat(String(value))
...
function conditionForOpenMeteoCode(value) {
  var code = parseInt(String(value), 10)
...
function openMeteoForecastDays(dailyForecastReport, todayString) {
  ...
  for (var i = 0; i < daily.time.length && result.length < 3; ++i) {
    ...
    result.push({
      ...
      precipitationProbability: daily.precipitation_probability_max ? roundedTemp(daily.precipitation_probability_max[i]) : "",
      precipitationMm: daily.precipitation_sum ? roundedDecimal(daily.precipitation_sum[i], 1) : "",
      uvIndex: daily.uv_index_max ? roundedDecimal(daily.uv_index_max[i], 1) : "",
      sunrise: daily.sunrise ? shortTime(daily.sunrise[i]) : "",
      sunset: daily.sunset ? shortTime(daily.sunset[i]) : "",
      maxWindKmph: daily.wind_speed_10m_max ? roundedTemp(daily.wind_speed_10m_max[i]) : "",
      maxWindMiles: daily.wind_speed_10m_max ? roundedTemp(daily.wind_speed_10m_max[i] * 0.621371) : ""
    })
  }
  return result
}

function openMeteoHourlyForecast(forecastReport, useImperial) {
  var hourly = forecastReport && forecastReport.hourly ? forecastReport.hourly : null
  if (!hourly || !hourly.time || !hourly.temperature_2m) return []

  var result = []
  for (var i = 0; i < hourly.time.length && result.length < 6; ++i) {
    var tempC = hourly.temperature_2m[i]
    result.push({
      time: shortTime(hourly.time[i]),
      temperature: roundedTemp(useImperial ? celsiusToFahrenheit(tempC) : tempC) + "°",
      precipitationProbability: hourly.precipitation_probability ? roundedTemp(hourly.precipitation_probability[i]) : "",
      icon: iconForOpenMeteoCode(hourly.weather_code ? hourly.weather_code[i] : null,
                                 hourly.is_day ? Number(hourly.is_day[i]) === 0 : false)
    })
  }
  return result
}
...
function openMeteoCurrentCondition(dailyForecastReport) {
  ...
  return {
    ...
    windDirection: compassDirection(current.wind_direction_10m),
    ...
    precipitationMm: roundedDecimal(current.precipitation, 1),
    cloudCover: roundedTemp(current.cloud_cover),
    pressureHpa: roundedTemp(current.pressure_msl),
    conditionText: conditionForOpenMeteoCode(current.weather_code),
