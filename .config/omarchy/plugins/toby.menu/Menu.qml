...
Item {
  ...
  readonly property var pinnedAppIds: ["brave-browser", "antigravity-ide", "org.gnome.Nautilus", "spotify"]

  function pinnedAppRank(appId) {
    var idx = root.pinnedAppIds.indexOf(appId)
    return idx >= 0 ? idx : root.pinnedAppIds.length
  }
  ...
  function mergeAppRows() {
    ...
    appRows.sort(function(a, b) { return root.pinnedAppRank(a.appId) - root.pinnedAppRank(b.appId) })
  ...
  function rebuildDisplay() {
    ...
    if (query) {
      ...
      // except for pinnedAppIds, which stay pinned to the top in that order.
      if (active === "apps") {
        rows.sort(function(a, b) {
          var aRank = root.pinnedAppRank(a.appId || "")
          var bRank = root.pinnedAppRank(b.appId || "")
